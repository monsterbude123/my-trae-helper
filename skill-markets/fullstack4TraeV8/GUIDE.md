...
Fullstack V8 是**文档驱动的全栈开发技能包**。核心思想：**先定义再开发，先文档再代码**。
...
graph LR
    ...
    CONTRACT --> PLAN["Plan"]
    PLAN --> DOCSYNC["DOC SYNC (🟡→🟢) ★"]
    DOCSYNC --> CLOSURE["Closure ★"]
    CLOSURE --> IMPL["Implement"]
    IMPL --> REVIEW["Review"]
    REVIEW --> DOCSYNC
    DOCSYNC --> ACCEPT["Accept"]
    ...
    style DOCSYNC fill:#ff6b6b,color:#fff
...
```
 ...
 5. 不文档同步不编码 (V8: 🟡→🟢 单写协议)    → DOC SYNC GATE 必须先过
...
- **DOC SYNC GATE**：编码前 Schema QA 检查 — KIT 脚本（文件存在）+ GATE 逻辑审查（内容一致性）。P0 内容必须已同步。V8 合并 DOC SYNC #1/#2 为单一 🟡→🟢 协议：编码前写（🟡），Review 后确认（🟢），同一份文档两阶段校验。
- **CONTRACT GATE**：编码前 Schema QA 检查 — KIT 脚本（contracts/ 齐全 + approved）+ GATE 逻辑审查（接口覆盖 + test 骨架）

两个门禁都必须通过才能编码。V8 升级为 KIT+GATE 双通道 + 单写协议。详见 [agents/implementer.md](agents/implementer.md) 步骤 0.5/0.7。
...
详见 [references/CHANGELOG.md](references/CHANGELOG.md) 的 V8.0 条目。核心变化：DOC SYNC #1/#2 合并为 🟡→🟢 单写协议。
