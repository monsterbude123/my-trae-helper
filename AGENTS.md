...
>
> **2026-08-21 蒸馏**:归档清理 + 经验沉淀 skill 入仓 — (1) 删除 `skill-markets/fullstack4TraeV9/` / `fullstack4TraeV10/` / `security-review/`(V9/V10 已被 V11 取代;旧 security-review 并入 trae-security-review);(2) 新增 `project-self-improving`(项目级 ERR/LEARN/FEATURE_REQUESTS)+ `user-self-improving`(个人级,可选 SOUL/TOOLS/MEMORY)两个经验沉淀 skill + `find-skills`(skill 发现,Vercel 同步);(3) `ai-testmate` 治理:补 bug-storage / input-router / openapi-to-testcases / v11-bug-flow-borrowed 四个 references + openapi-extractor.py / workspace-detect.py 两个脚本,改 agents 三件套;(4) `aigc-smart-kit` H3 子套件 SKILL.md 治理(i2v / ref2v / t2v / v2v / video-input-analyzer / video-prompt-method);(5) `skill-markets/MANIFEST.yaml` 同步清理残留条目;(6) `scripts/import_learnings.py` 入仓(未挂载,待后续);(7) `references/learnings-2026-08-21.json` 导出本次经验。**注册表同步**:`registry/skills.yaml` 新增 project-self-improving / user-self-improving 条目 + 移除 V9/V10/security-review;`scripts/<name>-guard.py` 三件套 + `.husky/<name>-gate` 三件套 完整闭环。参考 [skill-markets/project-self-improving/SKILL.md](skill-markets/project-self-improving/SKILL.md) + [skill-markets/user-self-improving/SKILL.md](skill-markets/user-self-improving/SKILL.md) + [references/learnings-2026-08-21.json](references/learnings-2026-08-21.json)。
>
...
> **2026-08-19 蒸馏**：vibe-coding-standards v2.5 行数守卫落地 — `scripts/vibe-coding-standards-line-guard.py`(封装 SKILL.md 100~350 弹性检查 + 5 Pillar 联动,Python stdlib + importlib 无 subprocess)+ `.husky/vibe-coding-standards-gate`(跨平台 husky 文件,detect-python.sh 探测)+ `.husky/pre-commit` 追加 step 9 + `registry/skills.yaml` 注册 guard+gate。**完整闭环验证**：fullstack4TraeV11 SKILL.md 644→349 行瘦身(5 段抽 `references/v11-*.md` — three-layer-control / fidelity-protocol / load-protocol / project-ecosystem / paths-config),自检三态真超限/真合规/合规瘦身后 全 PASS。**根因根治**:vibe-coding-standards/scripts/validate_vibe_docs.sh 从未注册 + 从未挂 husky gate 导致 V11.5.0→V12.0.0 全程未触发行数检查,本次 guard-smith 委派落地完整修复链。参考 [scripts/vibe-coding-standards-line-guard.py](scripts/vibe-coding-standards-line-guard.py) + [.husky/vibe-coding-standards-gate](.husky/vibe-coding-standards-gate) + [skill-markets/fullstack4TraeV11/CHANGELOG.md V12.0.0.P4](skill-markets/fullstack4TraeV11/CHANGELOG.md)。
>
> **2026-08-19 蒸馏**：agent-dev-control-kit SKILL.md 482 → 244 行瘦身(-49%)— 由本次会话"处理 agent-dev-control-kit/SKILL.md 超限问题"指令触发。**§1.5 地图 vs 规范判定**：内联保留 三层架构 ASCII / 整体流程 mermaid / 三层职责表 / 5 Execution + 5 Guard + 4 Gate 摘要表 / 联动 mermaid + 失败矩阵 / 5 条设计原则 / 目录结构一级 tree / §11 Gate 自验收(会话蒸馏硬约束,必须可见);指针化 已存在 references/ 内容 + scenarios/ 01-05 + 完整指标表 → 摘要 + 指针。**两道契约守卫生效**(不退让):`tests/catalogs/skill-catalog.yaml` required_sections 硬要求 §11 heading → 主标题 §10 → §11 + 子章节 §10.1.x / §11.2 / §11.3 → §11.1.x / §11.2 / §11.3(全5处);`skill-markets/MANIFEST.yaml` must_contain 硬要求 `## §0 定位` → 标题 §0 三层架构 → §0 定位(保留原貌)。**版本** 1.2.0 → 1.2.1(PATCH — 纯文档瘦身)。**验证**:`pytest -m trap` 31 passed + `pytest tests/unit/` 118 passed + `python scripts/catalog-guard.py` ✅。参考 [skill-markets/agent-dev-control-kit/SKILL.md](skill-markets/agent-dev-control-kit/SKILL.md) + [skill-markets/agent-dev-control-kit/CHANGELOG.md §Unreleased 1.2.1](skill-markets/agent-dev-control-kit/CHANGELOG.md)。
>
> **2026-08-19 蒸馏**：新建 `common-project-coding-conf` skill v1.0 — 接管 `project-rules-gate` v0.2 + `vibe-coding-routes`（未落地）的合并职责。三件套：①§1 路由表（场景关键词→必加载 skills：改代码/修 bug/提交/验收/理解代码/新建 skill 等 13 场景）；②§2 自检协议（6 项检查：skill 安装/description 触发词/gitnexus MCP 探活/索引新鲜度/项目 rules forge/市场 description 扫描，`scripts/cpcc-self-check.mjs` 一键跑）；③§3 forge 协议（保留 project-rules-gate 的 forge_project_rules_skill.py + templates + workflows + references，迁移 + 改 4 处引用名）。**同步修复 2 个历史问题**：`coding-xinfa` description 加触发词（之前"适用于所有项目"反而不触发）；`fullstack4TraeV11` SKILL.md 补全缺失的 description 字段。**删除整个 `skill-markets/project-rules-gate/` 目录**，其 forge 脚本/templates/workflows/references 已全量迁移到 cpcc。**关系明文化**：`project-rule-skill`（TRAE 工作区级产物名）= forge 产物；`project-rules-gate`（原制作者）= 已删除；`fullstack4TraeV11` = 聘用者（复用 [PROJECT-RULES-GATE] 协议）。**§1.11 写权范畴注册表更新**：cpcc 注册表条目 + `scripts/cpcc-guard.py` + `.husky/cpcc-gate` 三件套的注册属于 guard-smith 域，待委派子代理落地。参考 [skill-markets/common-project-coding-conf/SKILL.md](skill-markets/common-project-coding-conf/SKILL.md) + [skill-markets/common-project-coding-conf/scripts/cpcc-self-check.mjs](skill-markets/common-project-coding-conf/scripts/cpcc-self-check.mjs)。
