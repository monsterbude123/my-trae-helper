...
|-------|-----------|------|---------|
| **product-teardown** | `product-teardown/SKILL.md` | Orchestrator. Collects requirements, presents feature menu, confirms selections, delegates heavy work. | User says "analyze product X", "replicate Y", "competitive analysis" |
| **product-teardown-analyze** | `product-teardown-analyze/SKILL.md` | Deep analyzer. Receives product context, outputs 6-dimension structured analysis. | Called by orchestrator via `Task` tool |
| **product-teardown-prd** | `product-teardown-prd/SKILL.md` | PRD writer. Receives selected features + analysis, generates structured PRD. | Called by orchestrator via `Task` tool |
...
|----------|---------|---------|
| `templates/menu.md` | Orchestrator (Phase 3) | Feature menu with checkboxes, complexity labels, version recommendations |
| `templates/prd-lite.md` | prd Agent | Chapters 1-3: overview, architecture, detailed requirements |
| `templates/prd-full.md` | prd Agent | Chapters 4-7: non-functional, tech, ops, iteration plan |
