...
def detect_spec_mode(filepath: str) -> str:
...
def read_change_status(change_dir: str) -> dict:
    ...
    artifacts['specs'] = {
        ...
        "mode": detect_spec_mode(spec_path) if spec_exists else "none",
    }
    ...
    # 检测 _invalidated/ — 方向已变信号
    invalidated_dir = os.path.join(change_dir, '_invalidated')
    reset_mode = os.path.isdir(invalidated_dir)
    reset_warning = None
    if reset_mode:
        timestamps = []
        for entry in os.listdir(invalidated_dir):
            full = os.path.join(invalidated_dir, entry)
            if os.path.isdir(full):
                timestamps.append(entry)
        reset_warning = (
            f"⚠️ 检测到 _invalidated/ ({len(timestamps)} 次历史重置): {timestamps[:3]}"
            if timestamps else "⚠️ _invalidated/ 存在（旧版本已归档）"
        )
        if len(timestamps) > 3:
            reset_warning += " | _invalidated/ 膨胀，建议清理（只保留最近 3 次）"
