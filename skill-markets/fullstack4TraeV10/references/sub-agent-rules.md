...
> 关联: [artifact-lifecycle.md](artifact-lifecycle.md) 文档分层 + [bug-workflow.md](bug-workflow.md) 任务路由 + [doc-sync.md](doc-sync.md) 索引器范围 + [process-rot-analysis.md](process-rot-analysis.md) 反踩坑 6 条 + [acceptance-gates-v10.md](acceptance-gates-v10.md) 通过依据 3 类分层。

---

## 0. 主上下文 Skill 加载必读清单（V10.9 NEW — 防再放）

> 来源：2026-08-07 用户反馈（4 轮返工蒸馏）。
> 本节是**主上下文自己**的纪律，§1+ 才是子代理纪律。

主上下文加载任意 skill 后，**必读**以下 references（如该 skill 有）：

| 类别 | 必读文件 | 用途 |
|------|----------|------|
| 结构 | `references/project-structure.md` | 确认目录布局 vs 项目实际 |
| 生命周期 | `references/artifact-lifecycle.md` | 确认文档分层 vs 项目实际 |
| 子代理规则 | `references/sub-agent-rules.md` | 委派注入模板（本文件即此） |
| 验收 | `references/acceptance-gates-v10.md` | 4 维满分硬门禁 |
...
### 1.1 主动扫描 vs 任务执行读取区分（V10.8 NEW — 修正冲突 4/5，英文控制词）

> 修正 Reviewer 白名单含 `docs/reports/`(log 层) 与 "log 层不作依据" 冲突。
> 修正 Rot Detector 白名单 `docs/` 与 "禁读 process 层" 冲突。
...
- 索引器扫描范围详见 [doc-sync.md](doc-sync.md) §索引器范围白/黑名单
...
- **大索引文件禁读**（V10.8 NEW — 回流自 doc-updater）：`DOCSMAP.md` / `.docmap.json` / `.docindex.json` 等机器索引文件动辄数万行，直接 Read 会瞬间击穿上下文。查询文档始终用 `query-index.py --grab/--lookup/--file` 等工具，禁止直接 Read 这些文件
...
> 来源: agent-delegation-discipline / 大规模删除事故 / 进程管理协议 蒸馏。
> 通用方法论 + 反例 4 行结构（现象/根因/教训/来源）。

### 7. 委派注入模板（V10.8 升级 — 迁移至 SKILL.md §1.5）

> 委派注入的强制头部模板和场景化决策已迁移至 [SKILL.md §1.5](../SKILL.md)。
> 本文件不再重复，避免双源维护。本节以下 §8-§15 为委派纪律的执行细则。

### 8. 子代理产物三层独立验证（V10.8 NEW）
...
与 §8 三层验证配合: 先 git diff --stat 确认文件有变更 → 再 evidence 抽检确认内容真实。
...
铁律: **AI 描述 ≠ 真实像素**。Read PNG 工具返回的描述可能编造颜色/布局/文字/路径。详见 [acceptance-gates-v10.md](acceptance-gates-v10.md) §通过依据 3 类分层。
...
### 15. 进程管理 Process Supervisor（V10.8 NEW — 项目化，按需启用）
...
- 索引器范围白/黑名单 → [doc-sync.md](doc-sync.md) §索引器范围白/黑名单
- 反踩坑 6 条铁律（反临时指令自旋 / Probe 不要 Assume / 半截文件 / URL query / API metadata 三层 / 用户语气转硬）→ [process-rot-analysis.md](process-rot-analysis.md) §4
- 通过依据 3 类分层（后端/编译类 / UI 渲染类 / 用户视角类）→ [acceptance-gates-v10.md](acceptance-gates-v10.md) §通过依据 3 类分层
- Bug 诊断方法论（6 层排查 / 5 步流水线 / 采集 vs 解析 / e2e 先行 / 先验后做 / 并行诊断）→ [agents/debugger.md](../agents/debugger.md)

---

## 自检清单（V10.8 — 委派前判断已迁移至 SKILL.md §1.5）

> 委派前的"是否注入"判断已在 SKILL.md §1.5 场景化决策表中覆盖。
> 本清单仅保留子代理返回后的验证项。
