...
> 前置: TRAE Work 客户端已登录,工作区已打开本项目根目录。
> 路径变量: ${WORKSPACE} 由调度器在运行时替换为当前仓库根目录。
...
**输出位置**: 把本次运行的结果保存到 ${WORKSPACE}/logs/daily-vibe-coding/YYYY-MM-DD/ 目录 (如果当日目录不存在就 mkdir -p)
...
0.1 扫描历史:
  - ls ${WORKSPACE}/logs/daily-vibe-coding/  → 列出所有历史日期目录
  - Read ${WORKSPACE}/logs/daily-vibe-coding/INDEX.md (若存在)  → 拿到历史摘要索引
...
A.1 信息源 (按权威度优先):
  ...
  2. 仓库内已有素材:
     - ${WORKSPACE}/skill-markets/vibe-coding-standards/SKILL.md
     - ${WORKSPACE}/skill-markets/deep-research/SKILL.md
     - ${WORKSPACE}/skill-markets/fullstack4TraeV11/SKILL.md
...
A.3 产物: ${WORKSPACE}/logs/daily-vibe-coding/YYYY-MM-DD/external-report.md
  必含章节:
...
B.1 必跑命令:
  - python ${WORKSPACE}/skill-markets/trae-security-review/scripts/scan_skills_dir.py ${WORKSPACE}/skill-markets/
  - ls ${WORKSPACE}/skill-markets/ | wc -l
  - 抽样 Read 3~5 个 SKILL.md 的 YAML frontmatter
  - Read ${WORKSPACE}/SECURITY-MAP.md 末次更新时间
  - Read ${WORKSPACE}/skill-markets/CAPABILITY-MAP.md

B.2 产物: ${WORKSPACE}/logs/daily-vibe-coding/YYYY-MM-DD/self-audit.md
  ## 体检结果 / ## 发现的真问题 (HIGH/MED/LOW 三档) / ## 不做 / 暂缓的项 / ## 差异

B.3 产物: ${WORKSPACE}/logs/daily-vibe-coding/YYYY-MM-DD/upgrade-guid.md
  ## 历史建议处置 / ## 本日新增升级建议表 / ## 落地步骤 / ## 不建议做的事 / ## 长期演进方向

B.4 产物: ${WORKSPACE}/logs/daily-vibe-coding/YYYY-MM-DD/implementation-log.md
  空模板,等采纳方填

### PART C — ★★★ 建议清单 + 自我评估(关键)★★★

产物: ${WORKSPACE}/logs/daily-vibe-coding/YYYY-MM-DD/SUGGESTIONS.md
...
1. 写 ${WORKSPACE}/logs/daily-vibe-coding/YYYY-MM-DD/INDEX.md (本日目录清单 + SUGGESTIONS 摘要)
2. 更新 ${WORKSPACE}/logs/daily-vibe-coding/INDEX.md (总索引)
