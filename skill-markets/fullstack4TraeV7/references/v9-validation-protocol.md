...
```powershell
# 1. 确认技能包版本
ls "$env:USERPROFILE\.trae-cn\builtin_skills\fullstack4traev7\references\completion-report-protocol.md"
# 存在 → V9+，不存在 → V8-，必须先更新
...
python $env:USERPROFILE\.trae-cn\builtin_skills\fullstack4TraeV7\scripts\v9-self-check.py
```
