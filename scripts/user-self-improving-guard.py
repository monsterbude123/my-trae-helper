#!/usr/bin/env python3
"""
scripts/user-self-improving-guard.py — user-self-improving 项目侧薄壳守卫 (2026-08-21 guard-smith 委派落地)

设计目的:
  user-self-improving 是个人级补充入口 skill(替代 self-improving-agent 个人级角色),
  含 7 assets(LEARNINGS.md / ERRORS.md / FEATURE_REQUESTS.md / SOUL.md / TOOLS.md / MEMORY.md / SKILL-TEMPLATE.md)
  ...
  本守卫是项目侧薄壳入口 — 委托本文件做 6 项硬检查,确保发布前合规。
  遵循 AGENTS.md §1.11 铁律 11 — 项目侧 guard 必带。

用法:
  python scripts/user-self-improving-guard.py user-self-improving
...
_SKILL_DIR = _REPO_ROOT / "skill-markets" / "user-self-improving"

# vibe-coding-standards v2.5 行数弹性上限(与 scripts/vibe-coding-standards-line-guard.py 一致)
_SKILL_LINE_LIMIT = 350

# 必存在的 assets(个人级经验沉淀的 7 个数据文件 + 3 件个人表达 SOUL/TOOLS/MEMORY)
_REQUIRED_ASSETS = (
    "LEARNINGS.md",
    "ERRORS.md",
    "FEATURE_REQUESTS.md",
...
# 必存在的 references(10 个:4 agent 接入 + 5 主题 + 1 兼容迁移)
...
def _check_assets() -> tuple:
    """检查 (3) 7 assets 文件齐全(含 SOUL/TOOLS/MEMORY 个人表达三件)。"""
    ...
    return True, f"assets 7 项齐全 ({', '.join(_REQUIRED_ASSETS)})"


def _check_references() -> tuple:
    """检查 (4) 10 references 文件齐全。"""
...
def _check_scripts() -> tuple:
    ...
    rel_scripts = f"skill-markets/user-self-improving/scripts"
...
def _check_hooks_dir_removed() -> tuple:
    """检查 (6) hooks/ 目录已删除(原 OpenClaw handler 已搬到 references/legacy-openclaw-handler.md)。"""
    hooks_dir = _SKILL_DIR / "hooks"
    if hooks_dir.exists():
        # 若 hooks/ 存在但为空 → 仍算失败(应彻底删除,而非留空)
        try:
            contents = list(hooks_dir.iterdir())
        except OSError as e:
            return False, f"hooks/ 目录存在但不可读: {e}"
        if contents:
            return False, (
                f"hooks/ 目录仍存在且非空 ({len(contents)} 项): "
                "原 OpenClaw handler 已搬到 references/legacy-openclaw-handler.md,应彻底删除 hooks/"
            )
        return False, "hooks/ 目录仍存在(空目录)— 应彻底删除而非留空"
    # 同时确认 references/legacy-openclaw-handler.md 存在作为迁移目标
    legacy_ref = _SKILL_DIR / "references" / "legacy-openclaw-handler.md"
    if not legacy_ref.is_file():
        return False, (
            f"hooks/ 已删除(✓),但 references/legacy-openclaw-handler.md 也不存在 — "
            "迁移目标缺失,请补 references/legacy-openclaw-handler.md"
        )
    return True, f"hooks/ 目录已删除(✓),legacy handler 已搬到 references/legacy-openclaw-handler.md"


def check_user_self_improving(skill_path: str) -> dict:
    """user-self-improving 专属守卫 — 组合 6 项检查。"""
    errors: list = []
    warnings: list = []
    info: list = []
    ...
    checks = (
        ...
        ("assets 7 项齐全", _check_assets),
        ("references 10 项齐全", _check_references),
        ("scripts 5 项 + bash -n", _check_scripts),
        ("hooks/ 目录已删除", _check_hooks_dir_removed),
    )
...
def main() -> int:
    ...
    parser = argparse.ArgumentParser(
        description="user-self-improving 项目侧薄壳守卫 (guard-smith 委派落地)",
        allow_abbrev=False,
    )
    parser.add_argument(
        "skill_path",
    ...
    result = check_user_self_improving(args.skill_path)
    ...
    if warnings:
        ...
        for w in warnings:
            print(f"  - {w}")
    if not passed:
        print("\n❌ user-self-improving 守卫检查失败:")
        for e in errors:
            print(f"  - {e}")
        return 1

    print("\n✅ user-self-improving 守卫 PASS")
    return 0
