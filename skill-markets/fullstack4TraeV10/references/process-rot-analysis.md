...
### 4.5.6 项目特定误删补丁 — 主动判断缺失（V10.8 补丁更新）
...
## §5.5 代理腐烂（V10.9 NEW — 基于 2026-08-07 反馈蒸馏）

### rot #21 — 主上下文 Skill 加载即工作

**现象**：主上下文收到 "Use Skill: X" 指令后，只读 SKILL.md 主文件就进入工作模式

**根因**：缺失 §0.5 Skill 加载协议（详见 [SKILL.md §0.5](../SKILL.md) 主上下文自己的纪律）

**检测方法**：
- session-start.py 检测：加载 skill 后是否有 eferences/ 目录 LS 记录
- 子代理 prompt 头部：是否显式包含 [MUST] 必读 references 子文档

**修复**：在 SKILL.md §0.5 增加必读清单 + AskUserQuestion 反模式

### rot #22 — ≥2 轮同类返工不触发根因诊断

**现象**：用户对同一类问题（命名/编号/结构/首次进入 phase）反馈 ≥ 2 轮，AI 还在修补当前产物

**根因**：缺失 §7 反复返工根因诊断协议（详见 [clarify-checklist.md §7](clarify-checklist.md)）

**检测方法**：
- 会话历史：相同 root cause 出现 ≥ 2 次
- Completion Report：是否含"已根因诊断"字段

**修复**：触发 [clarify-checklist.md §7](clarify-checklist.md) 诊断协议 6 步

### rot #23 — 委派子代理"完成"未质疑性验收

**现象**：主上下文接收子代理"PASS"声明，未独立执行 grep/ls 验证

**根因**：缺失 Agent-子代理交互质量检查（详见 [sub-agent-rules.md §0](sub-agent-rules.md) 主上下文必读）

**检测方法**：
- 主上下文是否在子代理报告后执行 evidence 抽检 ≥ 1 个
- 是否独立 grep/ls 验证产物存在

**修复**：sub-agent-rules.md §0 主上下文必读清单强制质疑性验收
