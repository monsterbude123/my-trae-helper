---
name: fullstack4traev7
version: "7.1.0"
description: "全栈文档驱动开发技能包 v7.0 — DOC FIRST + Contract-First + Spec-Driven + TDD + Cockpit 驾驶舱 + 圆桌会议 + 技能生长。10 Agent 流水线（Cockpit→Intake→Proposal→Spec→[Prototype]→Contract→Plan→[DOC SYNC #1]→Implement→Review→[DOC SYNC #2]→Accept）+ 双层状态卡 + 7 维度量化验收 + feedback-loop 漂移回流 + 30% 需求原子化去重。当 agent 进行新功能开发、系统重构、多模块协作、前后端分离、需要协议先行 / TDD / E2E / 量化验收时加载。适用于中大型项目、多人协作、高质量高稳定性要求场景。简单 bug 修复或快速原型不强制走完整流水线。"
requires:
...
Phase 5.5: DOC SYNC #1  ★ 硬触发 doc-updater: 知识回流持久化文档
Phase 5.6: Closure-Define  ★ 新增，不可跳过 planner: 提取最小业务闭环链 → closure-checklist.md
...
Phase 7.5: DOC SYNC #2  ★ 硬触发 doc-updater: 二次同步 + 验证无 specs/changes/ 残留引用
Phase 8: Accept     acceptance-discipline: E2E + 性能 + 安全门禁 → 交付

🛑 不可跳过: Contract / Closure-Define / DOC SYNC #1 / DOC SYNC #2
...
```
1. 相位不可跳过  Contract / Closure-Define / DOC SYNC #1 / DOC SYNC #2 不可跳过，跳过=回退
...
| 原型设计规则 | [references/prototype-rules.md](references/prototype-rules.md) |
| 版本变更 | [references/CHANGELOG.md](references/CHANGELOG.md) |
| 迁移 V5 → V7 | [references/migration-guide.md](references/migration-guide.md) |
