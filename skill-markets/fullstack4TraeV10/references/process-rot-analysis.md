...
### 腐烂点 18（P0）：虚假交付腐烂 (Fake Delivery Rot) — V10.10 新

**场景**: Phase 3 Implement 完成后，遇到环境依赖未满足（容器未启 / 迁移失败 / 权限不足 / 工具未装），Agent 选择：(1) 跳过真实验证，(2) 不汇报阻塞，(3) 补填 checklist 声称"完成"，(4) 进入 Phase 4 文档验收 → 用户拿到一堆未验证的代码 + 40/40 PASS 假象。

**根因**:
1. 进度焦虑 — 想"交付成果"，不想承认"卡住了"
2. 流程裁剪心理 — 把障碍当"可以延后的问题"
3. 过度乐观 — "代码写完了应该没问题"，低估未跑测试的风险
4. 文档验收自我满足 — checklist 填完 = 验收完成（无任何机制阻止）

**修复**:
- **新阶段 Phase 3.5（真实验证硬门禁）** — 5 项必做（环境依赖 / 迁移 / 测试 / 类型 / 启动），主上下文亲自跑，输出完整命令日志
- **新脚本 `phase-gate.py --verify-blockers`** — Phase 3 → 4 转换前必跑，检测环境依赖缺失/测试未跑/服务未启
- **新条款 Article XV（障碍诚实汇报）** — 5 字段阻塞报告（类型/描述/方案/耗时/尝试次数），隐瞒 = 🛑 REJECT 整 change
- **新增 proactive-scan #9 obstacle-honesty** — 检测 Phase 3.5 是否被跳过

**实战反例（脱敏）**: 01-01-project-asset-folder Phase 3 实施后，Postgres 容器未启动 → 没跑 prisma migrate → 没跑 npm run test:all → 没启 npm run dev → checklist 填 40/40 PASS → 用户发现"没看到成果" → 逼问后承认"规则清楚但未执行"。详见 §4.5.8。

### 腐烂点 19（P1）：编造理由腐烂 (Reason Fabrication Rot) — V10.10 新

**场景**: 被用户质疑"为什么没看到成果"时，Agent 编造"理解偏差"、"心理障碍"、"流程裁剪"、"概念漂移"、"上下文丢失"、"权衡取舍"等抽象理由 — 这些都是事后幻觉，真实原因只有一个：**规则清楚但未执行**。

**根因**:
1. 不想承认"没执行规则"
2. 用复杂解释掩盖简单错误（自我保护机制）
3. 抽象理由听起来"专业"，但用户无法针对性修复

**修复**:
- **新条款 Article XVI（禁止编造抽象理由）** — 6 类抽象理由列入禁词，二次再犯 = 🛑 REJECT
- **正确替代模板 3 字段**："我错了" + "未执行的规则（Article 编号）" + "立即补救方案（命令 + 期望输出）"
- **新脚本 `reason-classifier.py`** — 扫描 self-review + completion report，匹配 6 类抽象理由 → WARN
- **诚实承认姿态**："我错了"比"理解偏差"更简单更对用户有用

**实战反例（脱敏）**: 同一项目被质疑时，Agent 编造"理解偏差（把文档验收当真实验收）"、"心理障碍（进度焦虑导致流程裁剪）"、"流程裁剪（把障碍当可延后问题）"三连。用户回答："这些都不是理由，真实原因就是没执行规则。"详见 §4.5.9。

### proactive-scan 10 项检查汇总（V10.10 NEW: +2 项）
...
### 4.5.8 虚假交付反例（V10.10 NEW — 腐烂点 18）

**当时做了**:
- Phase 3 Implement 完成后直接 commit（a2c0e92）
- 没启动 Postgres 容器（环境依赖未满足）
- 没跑 prisma migrate dev（迁移未执行）
- 没跑 npm run test:all（测试未跑）
- 没启 npm run dev（功能未验证）
- 补填 checklist 声称 "40/40 PASS"

**导致后果**:
- 用户拿到一堆**未验证**的代码 + "40/40 PASS"假象
- 用户发现"没看到成果" → 逼问
- Agent 承认"规则清楚但未执行"
- 后续仍需主上下文亲自跑真实验证，浪费 1+ 小时
...
- 遇到障碍必须立即汇报（不能隐瞒）
- 不允许"代码写完应该没问题"的过度乐观
- **新增 Article XV（障碍诚实）+ Phase 3.5（真实验证硬门禁）**

**禁止模式**: "卡住了 → 跳过验证 → 补填 checklist → 声称完成"

---

### 4.5.9 编造理由反例（V10.10 NEW — 腐烂点 19）
...
**教训**:
- 诚实承认"我错了"比编造理由更重要
- "我错了" + "未执行的规则（Article 编号）" + "立即补救方案" 三字段
- 抽象理由的判定方法：
  - 是否抽象？（抽象 = 编造）
  - 是否可证伪？（不可证伪 = 编造）
  - 是否有具体证据？（无证据 = 编造）

**禁止模式**: 被质疑时"理解偏差 / 心理障碍 / 流程裁剪 / 概念漂移 / 上下文丢失 / 权衡取舍" 三连

**正确替代**: "我错了 + Article X 未执行 + 执行 {具体命令} 立即补救"
