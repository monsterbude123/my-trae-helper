...
| **V11.8.7 gitnexus 三件套** | **2026-08-18** | **用户 14 次质问蒸馏:fullstack4TraeV11 主上下文/子代理从未真调 gitnexus MCP(仅文字声明)→ 落 3 件硬约束:①scripts/gitnexus-trace.py 写 .trae/logs/gitnexus-trace.jsonl(.trae/ 不提交);②sub-agent-rules §7 [GITNEXUS] 从提示符改 run_mcp 咒语 + 反例 §7.1;③commit-minimum-check.py 加 #6 check_gitnexus_invocation_trace(--allow-stale-gitnexus 兜底,24h 内有 ok=true 才 PASS)** | **[scripts/gitnexus-trace.py](fullstack4TraeV11/scripts/gitnexus-trace.py) + [references/sub-agent-rules.md §7 反例 §7.1](fullstack4TraeV11/references/sub-agent-rules.md) + [scripts/commit-minimum-check.py 6 项检查](fullstack4TraeV11/scripts/commit-minimum-check.py) + [registry/skills.yaml fullstack4TraeV11-gitnexus 条目](registry/skills.yaml) + [.husky/pre-commit 步骤 8](.husky/pre-commit)** |
| **V11.8.7.P2 项目级 rules 三件套** | **2026-08-18** | **用户追问 fullstack4TraeV11 指导项目 agent 初始化建立 project rules skills 的内容 → 落 3 件:①templates/project-rules-skill-template/SKILL.md 加 §0 三件套铁律 + §3.5 7 维 checklist + §5 用户通知 markdown 块 + §7 4 条新反例;②templates/project-rules-example/README.md 加三件套章节 + 5 条具体反例 + 7 维 checklist 表 + 全 5 rules 兜底模板;③V11 SKILL.md §14.1.1 NEW + §14.2 触发词表 + §14.3 反例表(V11.8.7 三件套入口)** | **[templates/project-rules-skill-template/SKILL.md](fullstack4TraeV11/templates/project-rules-skill-template/SKILL.md) + [templates/project-rules-example/README.md](fullstack4TraeV11/templates/project-rules-example/README.md) + [SKILL.md §14.1.1](fullstack4TraeV11/SKILL.md) + CHANGELOG V11.8.7.P2 条目** |
