...
#   (non-zero exit / 常见错误关键词),若有 → 输出 reminder
#   提示用户/agent 考虑写一条 ERR entry 到
#   $HOME/.user-self-improving/.learnings/ERRORS.md。
#
# 输入约定:
#   Trae/Claude Code 把工具输出以 stdin/CLAUDE_TOOL_OUTPUT 环境变量传入。
#   本脚本同时支持 stdin 与环境变量(Trae 可能用其中之一)。
#
# 输出格式(stdout):与 activator 同款,XML-style reminder block。
#
...
cat <<REMINDER
<user-self-improving-error-detected>
Source: $INPUT_SOURCE
Signal: $FOUND
</user-self-improving-error-detected>

<user-self-improving-error-reminder>
Tool output contained an error signal. Consider logging it to
\`\$HOME/.user-self-improving/.learnings/ERRORS.md\`:
...
See \`skill-markets/user-self-improving/SKILL.md §5\` for the full format.
</user-self-improving-error-reminder>
REMINDER
