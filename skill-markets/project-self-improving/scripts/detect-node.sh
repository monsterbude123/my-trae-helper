#!/usr/bin/env bash
# scripts/detect-node.sh — 跨平台 Node.js 探测 + 能力校验(供 Git Hook / hook-self-check 复用)
#
# 设计原则(2026-08-21 项目-side,基于 detect-python.sh 同款协议):
#   1. 跨平台: PATH + 平台典型位置(由 uname 动态生成),带能力校验
#   2. 仅信 PATH + 平台典型安装位置,绝不写死具体路径
#   3. 能力校验失败 → 报错并退出(不静默降级到 stub,违反 §1.11 gate 自验收铁律)
#   4. 导出到 MY_NODE 供子脚本拾取
#
# 用法(在 .husky/pre-commit / .trae/hooks.json 子命令头部):
#   . "$(dirname "$0")/../scripts/detect-node.sh" || exit 1
#
...
_candidates() {
  # Git for Windows 新版 MSYS 的 uname 返回 Linux(而非 MINGW*),用"文件系统可达性"兜底:
