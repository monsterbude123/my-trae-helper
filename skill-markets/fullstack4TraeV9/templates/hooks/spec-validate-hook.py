#!/usr/bin/env python3
"""spec-validate-hook.py — V9.2 Spec 格式校验
PostToolUse Hook: 写 spec.md 后自动检查 Delta Spec 格式 + prototypes/ 完整性。

V9.2 变更:
  - 支持 Delta Spec（ADDED/MODIFIED/REMOVED 段）
  - prototypes/ 检查改为两份文档（design-prompt.md + ui-ux-logic.md）
  - 移除 L0-L4 编号检查（V9.2 不再使用）
  - 移除 change_name 旧命名
"""
...
if has_ui:
    ...
    if m:
        ...
        if not proto_dir.exists() or (not has_design and not has_ux):
            issues.append(
                f"涉及 UI 但 prototypes/ 缺少 V9.2 文档: "
